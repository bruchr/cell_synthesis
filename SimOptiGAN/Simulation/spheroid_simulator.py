...
from utils import prepare_and_save_mask, delete_mask
...
class SpheroidSimulator:
    ...
    def missing_param(params: dict) -> Optional[str]:
        ...
        necessary_keys = [
            ...
            "num_of_images", "max_overlap", "min_dist", "break_criterion_positions", "break_criterion_objects",
            ...
            "px_size_phantom_img", "px_size_sim_img", "px_size_desired", "input_fragment_folder", "fragment_probability",
            "input_subculture_folder", "num_of_subcultures", "subculture_radius_xy", "subculture_radius_z",
            "subculture_min_dist", "px_size_subculture_phantoms", 
            ]
    ...
    def import_settings(self, params: dict) -> None:
        ...
        if len(params["generated_image_shape"]) != 0:
            ...
            self.shapes_des = [np.asarray(tiff.imread(path_spher_mask).shape) for path_spher_mask in params["input_spheroid_mask"]]
        self.shapes_sim = []
        ...
        self.generation_params = {
            ...
            "path_fragment_folder": params["input_fragment_folder"],
            "fragment_probability": params["fragment_probability"],
    ...
    def run(self, verbose: bool = False, save_interim_images: bool = False) -> None:
        ...
        for n_mask, path_spher_mask in enumerate(self.path_spheroid_mask_list):
            ...
            for n in range(1, self.num_of_images + 1):
                ...
                sl_z_append = slice(None) if self.generation_params['z_append'] == 0 else slice(None, -self.generation_params['z_append'])
                label = label[sl_z_append, :, :]
