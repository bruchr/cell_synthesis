...
class CycleGANDataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.train:
            self.Seg_paths = paths_images_in_folder(self.folder, self.opt["mode"], "Seg")
            self.Seg_size = len(self.Seg_paths)
            if not self.train or opt["virtual_dataset_multiplicator"] == 1 or opt["virtual_dataset_multiplicator"] is None:
                self.Seg_size_virt = len(self.Seg_paths)
            else:
                self.Seg_size_virt = np.round(len(self.Seg_paths) * opt["virtual_dataset_multiplicator"])
            self.Seg_transforms = get_transforms(self.opt)
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            Seg_path = self.Seg_paths[random.randint(0, self.Seg_size-1)]  # get random index for A to avoid fixed pairs        
        ...
        if self.train:
            ...
            samples_Seg = self.Seg_transforms({'image': Seg_img})
            
        if self.train:
            return {'A': samples_A['image'], 'B': samples_B['image'], 'Seg': samples_Seg['image'], 'A_path': A_path, 'B_path': B_path}
        elif self.a2b:
